...
import (
        ...
        "github.com/gatewise/gatewise/internal/simulator"
...
func NewServer(pgStore *store.PostgresStore, factory *connector.Factory, rec *reconciler.Reconciler, licenseMgr *license.Manager) *Server {
        ...
        s := &Server{
                ...
                simulator:  simulator.NewSimulator(factory),
        }
...
func (s *Server) setupRoutes() {
        ...
        api := s.router.Group("/api")
        {
                ...
                policies := api.Group("/policies")
                {
                        ...
                        policies.POST("/simulate", s.simulatePolicy)
                        policies.POST("/:name/simulate", s.simulatePolicyUpdate)
                }
...
// simulatePolicy simulates a new policy without applying it
func (s *Server) simulatePolicy(c *gin.Context) {
        var req struct {
                YAML   string         `json:"yaml"`
                Policy *models.Policy `json:"policy"`
        }
        ...
        // Parse policy from YAML or use provided policy
        var policy *models.Policy
        var err error

        if req.YAML != "" {
                policy, err = s.parser.ParseYAML([]byte(req.YAML))
                if err != nil {
                ...
                c.JSON(http.StatusBadRequest, gin.H{"error": "Either yaml or policy must be provided"})
                return
        }
        ...
        // Run simulation
        result, err := s.simulator.Simulate(c.Request.Context(), policy)
        if err != nil {
        ...
        c.JSON(http.StatusOK, gin.H{
                "simulation": result,
                "summary": gin.H{
                        "willCreate": len(result.WillCreate),
                        "willModify": len(result.WillModify),
                        "willDelete": len(result.WillDelete),
                        "accessGrants": len(result.AccessGrants),
                        "accessRevoke": len(result.AccessRevoke),
...
// simulatePolicyUpdate simulates updating an existing policy
func (s *Server) simulatePolicyUpdate(c *gin.Context) {
        name := c.Param("name")
        ...
        // Get existing policy
        oldPolicy, err := s.store.GetPolicy(c.Request.Context(), name)
        if err != nil {
                c.JSON(http.StatusNotFound, gin.H{"error": "Policy not found"})
                return
        }
        ...
        if req.YAML != "" {
                newPolicy, err = s.parser.ParseYAML([]byte(req.YAML))
                if err != nil {
                        c.JSON(http.StatusBadRequest, gin.H{"error": fmt.Sprintf("Failed to parse YAML: %v", err)})
                        return
                }
        ...
        // Validate new policy
        if err := s.validator.Validate(newPolicy); err != nil {
                c.JSON(http.StatusBadRequest, gin.H{"error": fmt.Sprintf("Validation failed: %v", err)})
                return
        }

        // Compare policies
        result, err := s.simulator.ComparePolicies(c.Request.Context(), oldPolicy, newPolicy)
        if err != nil {
        ...
        c.JSON(http.StatusOK, gin.H{
                ...
                "oldPolicy": gin.H{
                        ...
                        "team": oldPolicy.Spec.Team,
                },
                "newPolicy": gin.H{
                        "name": newPolicy.Metadata.Name,
                        "team": newPolicy.Spec.Team,
                },
                "summary": gin.H{
                        "willCreate": len(result.WillCreate),
                        "willModify": len(result.WillModify),
